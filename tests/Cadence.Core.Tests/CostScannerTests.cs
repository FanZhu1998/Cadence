...
public class CostScannerTests
{
    ...
    [Fact]
    public void Pricing_ShippedTableLoadsAndCoversTheModelsWeExpect()
    {
        var table = PricingTable.Load(overridePath: "does-not-exist.json");

        Assert.True(table.ModelCount > 5);
        Assert.NotNull(table.LastUpdated);
        Assert.NotNull(table.For("claude-opus-5"));
        Assert.NotNull(table.For("gpt-6"));
    }

    [Fact]
    public void Pricing_DatedModelIdsResolveByLongestPrefix()
    {
        // Real model ids carry dated suffixes the table does not list, so exact-match-only lookup
        // would price most real traffic at zero.
        var table = PricingTable.Load(overridePath: "does-not-exist.json");

        Assert.NotNull(table.For("claude-opus-5-20260814"));
        Assert.Equal(table.For("claude-opus-5"), table.For("claude-opus-5-20260814"));
    }

    [Fact]
    public void Pricing_ChargesTheOneHourCacheWriteAboveTheFiveMinuteOne()
    {
        var table = PricingTable.Load(overridePath: "does-not-exist.json");

        var short5m = new CostEntry
        {
            Provider = ProviderId.Claude, EntryId = "a", Timestamp = DateTimeOffset.UnixEpoch,
            Model = "claude-opus-5", CacheWrite5mTokens = 1_000_000,
        };
        var long1h = short5m with { CacheWrite5mTokens = 0, CacheWrite1hTokens = 1_000_000 };

        Assert.True(table.CostOf(long1h) > table.CostOf(short5m),
            "the 1h cache write must not be priced at the 5m rate");
    }

    [Fact]
    public void Pricing_UnknownModelCostsZeroAndIsFlagged()
    {
        var table = PricingTable.Load(overridePath: "does-not-exist.json");
        var entry = new CostEntry
        {
            Provider = ProviderId.Claude, EntryId = "a", Timestamp = DateTimeOffset.UnixEpoch,
            Model = "some-model-released-tomorrow", InputTokens = 1_000_000,
        };

        Assert.Equal(0m, table.CostOf(entry));
        Assert.True(table.IsUnpriced(entry.Model));
    }

    [Fact]
    public void Pricing_UserOverrideWinsPerModelWithoutRestatingTheTable()
    {
        var path = Path.Combine(Path.GetTempPath(), $"cadence-pricing-{Guid.NewGuid():N}.json");
        File.WriteAllText(path,
            """{ "models": { "claude-opus-5": { "input": 99.0, "output": 99.0, "cacheRead": 0, "cacheWrite5m": 0 } } }""");

        try
        {
            var table = PricingTable.Load(overridePath: path);

            Assert.Equal(99.0m, table.For("claude-opus-5")!.InputPerMillion);
            // Everything not overridden still comes from the shipped table.
            Assert.NotNull(table.For("claude-sonnet-5"));
        }
        finally
        {
            File.Delete(path);
    ...
    [Fact]
    public void Pricing_ABrokenOverrideDoesNotTakeCostsOffline()
    {
        var path = Path.Combine(Path.GetTempPath(), $"cadence-pricing-{Guid.NewGuid():N}.json");
    ...
    [Fact]
    public void Pricing_CostArithmeticIsPerMillionTokens()
    {
        var table = PricingTable.Load(
            builtInJson: """{ "models": { "test": { "input": 10.0, "output": 30.0, "cacheRead": 1.0, "cacheWrite5m": 12.0 } } }""",
            overridePath: "does-not-exist.json");

        var entry = new CostEntry
        {
            Provider = ProviderId.Claude, EntryId = "a", Timestamp = DateTimeOffset.UnixEpoch, Model = "test",
            InputTokens = 1_000_000, OutputTokens = 500_000, CacheReadTokens = 2_000_000, CacheWrite5mTokens = 250_000,
        };

        // 10 + 15 + 2 + 3
        Assert.Equal(30.0m, table.CostOf(entry));
    }

    [Fact]
    public void ShippedPricingFileIsValidJson()
    {
        // Guards against a hand-edit breaking the embedded table, which would silently zero costs.
        var table = PricingTable.Load(overridePath: "does-not-exist.json");
        Assert.True(table.ModelCount > 0, "the embedded pricing table failed to parse");

        var pricing = table.For("claude-opus-5")!;
        Assert.True(pricing.InputPerMillion > 0);
        Assert.True(pricing.CacheWrite1hPerMillion > pricing.CacheWrite5mPerMillion);
    }
}
